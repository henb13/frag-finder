...
module.exports = async function getFrags(pathToJsonFiles, playerChosen = false) {
    ...
    for (let i = 0; i < demoFiles.length; i++) {
        ...
        const allNotableClutchesInMatch = matchData.players
            .filter(player =>
                player.clutches.some(clutch => clutch.has_won && clutch.opponent_count >= 3)
            )
            .map(player => {
                return player.clutches.map(({ opponent_count, round_number }) => ({
                    player: player.name,
...
function hasNotableDeagleFrags(kills) {
    return kills.some(
        ({ weapon, is_headshot }) => weapon.weapon_name === "Desert Eagle" && is_headshot
    );
}
